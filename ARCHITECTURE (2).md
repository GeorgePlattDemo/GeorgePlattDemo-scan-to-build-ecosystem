...
**Companion to `home-twin-app.html`.** The HTML file is a working MVP whose internal layers (Schema → DB → Repositories → API → Orchestrator → UI) are deliberately structured so each maps one-to-one onto the production system described here. Replacing the in-browser layer with a real server is a transport change, not a rewrite.
...
Relationships, precisely: the **Ontology Context publishes a versioned Published Language** (`packages/ontology`) that Record, Orchestration, and Channel contexts consume read-only — no context may mutate the vocabulary it validates against. The **Record Context is upstream** of Orchestration: orchestration reads projections and writes results back only as events. Every external party sits behind an **anti-corruption layer** in the Channel Context: a vendor's quote format, a store's SKU scheme, an installer's milestone vocabulary are translated at the adapter and never leak inward. Settlement and Documents are **conformist** consumers of record events. This map is why "no actor replaces the owner as record holder" is structurally true rather than policy: nothing outside the Record Context can write to the record except through events the orchestrator validates.
...
The MVP's `api.*` methods map one-to-one onto these endpoints; the UI never needs to change when the transport does.
...
The single most important package remains `packages/schema/workPacket.ts` — versioned, validated identically on client, server, and channel boundary, never owned by any one service — now joined by `packages/ontology`, the published language every context conforms to.
